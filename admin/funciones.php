...
//funcion para agrear un nuevo provincia a la BD
function agregarNuevoprovincia($provincia){
    include("conexion.php");
    //armamos el query para insertar en la tabla provincia
    $query = "INSERT INTO provincia (id, nombre_provincia)
    VALUES (NULL, '$provincia')";

    //insertamos en la tabla provincia
    if (mysqli_query($conn, $query)) { //Se insertó correctamente
        $mensaje = "provincia agregado correctamente";
    } else {
        $mensaje = "No se pudo insertar en la BD" . mysqli_errno($conn);
    }
    return $mensaje;
